...
public interface LaboratoryDao {
    ...
    @Insert("insert into laboratory(name,campus,admin,admin_number,content,create_date,is_free) values(#{name},#{campus},#{admin},#{adminNumber},#{content},#{createDate},#{isFree})")
    int insertLaboratory(Laboratory laboratory);
    @Select("select * from laboratory where id = #{id}")
    Laboratory getLaboratoryById(int id);
    @Update("update laboratory set name = #{name},campus = #{campus},admin = #{admin},admin_number = #{adminNumber},update_date = #{updateDate},content = #{content},is_free = #{isFree} where id = #{id}")
