...
public interface ExperimentDao {
    ...
    @Insert("insert into experiment(name,number,content,task,create_date) values(#{name},#{number},#{content},#{task},#{createDate})")
    ...
    @Update("update experiment set name = #{name},number = #{number},content = #{content},task = #{task} where id = #{id}")
    void updateExperiment(Experiment experiment);
    @Update("update experiment set name = #{name},number = #{number},content = #{content} where id = #{id}")
